...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Faithful path: the deck was already generated by the presentation
    // agent (generate-full saves _agentSlides). Derive the structured deck
    // 1:1 from it — same order, same copy, same images — instead of
    // REGENERATING a different deck from the brief (the old "two brains"
    // behavior that made /edit and the export mismatch the generated deck).
    const agentSlides = data._agentSlides as Array<{ slideType: string; title: string; content?: Record<string, unknown> }> | undefined
    if (agentSlides?.length) {
      ...
      console.log(`[gamma-proto] derived structured deck 1:1 from agent slides (${presentation.slides.length} slides) — no regeneration`)
      return NextResponse.json({
        success: true,
        brandName: presentation.brandName,
        slideCount: presentation.slides.length,
        presentation,
        derivedFromAgentDeck: true,
      })
    }
