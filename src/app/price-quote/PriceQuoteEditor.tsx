...
export default function PriceQuoteEditor({
  initialData,
  ...
  const draftVersionRef = useRef<number>(initialDraftVersion ?? 1)
  const [saveState, setSaveState] = useState<'idle' | 'saving' | 'saved' | 'conflict' | 'error'>('idle')
  const savedSnapshotRef = useRef<string>(JSON.stringify(initialData ?? defaultData))
  ...
  // ── Preview via POST /preview → blob URL (replaces the ?data= query-string
  //    preview, which was IDOR-adjacent and URL-length-capped). ──
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const res = await fetch('/api/price-quote/preview', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ data, page: previewPage }),
        })
  ...
  }, [data, previewPage, previewKey])

  // ── Debounced autosave. Creates the row on first change, then PATCHes with an
  //    optimistic lock. A 409 means someone else saved; we surface it and reload
      ...
      try {
        if (!quoteId) {
          const res = await fetch('/api/price-quotes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ title: data.clientName || 'הצעת מחיר', draft_data: data }),
          })
          if (!res.ok) throw new Error('create failed')
          const created = await res.json()
          setQuoteId(created.id)
          draftVersionRef.current = created.draft_version
        ...
        const res = await fetch(`/api/price-quotes/${quoteId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ data, expected_draft_version: draftVersionRef.current }),
        })
        if (res.status === 409) {
        ...
        draftVersionRef.current = saved.draft_version
        savedSnapshotRef.current = snapshot
        setSaveState('saved')
      } catch {
        setSaveState('error')
  ...
  // Reload the server's winning version after a 409 conflict.
  const reloadFromServer = useCallback(async () => {
    if (!quoteId) return
    const res = await fetch(`/api/price-quotes/${quoteId}`)
    if (!res.ok) return
    const payload = await res.json()
    draftVersionRef.current = payload.draft_version
    savedSnapshotRef.current = JSON.stringify(payload.draft_data)
    setData(payload.draft_data)
    setSaveState('saved')
  }, [quoteId])
      ...
      <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-gray-800">
            הצעת מחיר
            {data.quoteNumber ? <span className="text-gray-400 font-normal text-base"> · {data.quoteNumber}</span> : null}
          </h1>
          <SaveChip state={saveState} onReload={reloadFromServer} />
        </div>
...
function SaveChip({
  ...
  if (state === 'conflict') {
    return (
      <button
        onClick={onReload}
        className="text-xs px-2 py-1 rounded-full bg-amber-100 text-amber-800 border border-amber-300 hover:bg-amber-200"
        title="ההצעה נערכה במקום אחר"
      >
        נערך במקום אחר — טען מחדש
      </button>
    )
  }
  const label =
    state === 'saving' ? 'שומר…' : state === 'saved' ? 'נשמר' : 'שגיאת שמירה'
  const cls =
    state === 'saving'
