...
**החלטה כמה שקפים — תלוי בעושר הבריף, לא במספר קבוע:**
- בסיס חובה = 14 שקפי הליבה. אסור לרדת מתחת.
- בריף דק (פסקה אחת, ללא מתחרים, ללא timeline, ללא platform mix): השאר על 14.
- בריף בינוני (יש מתחרים *או* פלטפורמות מוגדרות *או* timeline): 15-16.
- בריף עשיר (מתחרים + פלטפורמות + timeline + תמונות client אותנטיות + ניחוח חזק): עד 17-18.
- כל שקף אופציונלי שמתווסף **חייב** להיות מבוסס דאטה אמיתי מהבריף או מה-research. אסור למתוח שקף ריק כדי להגיע למספר. אם אין מה לכתוב על timeline — אל תכלול אותו, גם אם יוצא 14 שקפים.
- סדר עדיפויות להוספה: (1) competitiveAnalysis אם יש מתחרים, (2) mediaMix אם יש platform breakdown, (3) timeline אם יש תאריכים, (4) contentPillars אם יש 3+ נושאים, (5) moodBoard אם יש תמונות client אותנטיות.
...
- CLOSING: רק כותרת + tagline. דוגמה: title="בואו נתחיל", tagline="Leaders × ${brandName}"
  ...
  const plannerAttempts = [
    ...
    { provider: 'gemini', model: 'gemini-3.7-flash', label: 'Gemini 3 Flash (retry)' },
  ]
