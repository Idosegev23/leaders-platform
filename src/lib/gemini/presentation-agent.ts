...
- metrics ≠ results: שני שקפי נתונים סמוכים לא חוזרים על אותם מספרים. metrics = הגדרת ה-KPIs (מה מודדים: Reach/Views/ER/Saves). results = תחזית שונה: פירוק ה-reach לפי משפיען (524K+353K+98K+88K → הסכום), או benchmark/ROI. אסור לשגר פעמיים את אותה סדרת מספרים.
- לכידות pillar: גוף שקף pillar מפתח את המושג שבכותרתו. שקף strategy/overview מסכם בשורה אחת לכל pillar — לא משכתב את גופי ה-pillars שאחריו. [רע: כותרת "בריאות ועמידות" עם גוף על אותנטיות.]
- שמות משפיענים בכותרת: שם עברי נקי בלבד (מהנתונים), בלי אמוג'י, בלי כפילות אנגלית/עברית, בלי handle גולמי. תווית תפקיד ניטרלית/מותאמת ("משפיען/ית"). כל פרופיל = משפיען אחד עם מספר עוקבים; אם מקבצים כמה — כל אחד מקבל מספר משלו.
- חשבון תוצרים↔נבחרת: כמויות התוצרים מתיישבות עם מספר המשפיענים (למשל 3 סטוריז × N יוצרים). כל יוצר שהוזכר בנבחרת מופיע גם ב-timeline. אל תשאיר יוצר בלי שיבוץ.
</proposal_integrity>
