...
## עקרונות תמונות — היררכיית עדיפויות חובה:

**סדר העדיפויות לכל slot של תמונת מוצר** (מהטוב לפחות-טוב):
1. **\`productScene1/2/3\`** מ-\`<generated_images>\` — אלה תמונות שעוברו עיבוד Nano Banana: ה-מוצר האמיתי של המותג מוטמע לתוך scene דק-איכותי 1920×1080 עם תאורה תואמת. **זה ה-primary**. אם זה זמין — השתמש בזה ולא במשהו אחר.
2. **תמונת \`heroImage\` אמיתית** מ-\`<scraped_assets>\` — אם זה תמונת hero/lifestyle ולא תמונת מוצר על רקע לבן.
3. **\`activityImage\`** מ-\`<generated_images>\` — AI gen עם הלוגו של המותג מוטמע בפינה.
4. **תמונת מוצר גולמית** מ-\`<scraped_assets>\` — **last resort**. תמונות מוצר מהאתר הן בדרך כלל מצומצמות ובסטייל catalog (white bg) שלא מתאים למצגת. השתמש בזה רק אם אין כלום אחר, ורק כ-backgroundImage על numbered-stats עם opacity:0.28 (לא כתמונה ראשית).

**חלוקת ה-productScenes** (אם קיימים שלושתם):
- **bigIdea** → \`productScene1\` (המוצר ב-foreground של scene)
- **deliverables** → \`productScene2\` (hero shot עם soft background)
- **closing/risks** → \`productScene3\` (integrated daily routine)

אם יש רק \`productScene1\` — השתמש בו ב-bigIdea בלבד; ל-deliverables עבור ל-three-pillars-grid טקסטואלי או heroImage אמיתית.

**חלוקת ה-slots לפי slide type:**
- **bigIdea**: productScene1 (חובה אם זמין). אחרת layout טקסטואלי (centered-insight) — אסור AI gen גנרי של "משחה כללית".
- **deliverables**: productScene2 ב-split-image-text. אחרת three-pillars-grid עם sideImage של heroImage אמיתי.
- **brief**: heroImage אמיתי מ-scraped אם יש; אחרת AI brand.
- **audience**: lifestyleImage אמיתי אם יש; אחרת AI audience.
- **caseStudies**: hero_image מ-<leaders_case_studies> אם יש; אחרת numbered-stats טקסטואלי.
- **closing**: productScene3 או heroImage אמיתי או AI cover.
- **goals/strategy/risks (three-pillars-grid)**: sideImage = lifestyleImage או productScene רלוונטי. שקף text-only כשתמונה זמינה = כשל.
- **metrics/nextSteps (numbered-stats)**: backgroundImage = lifestyleImage או heroImage. סטטיסטיקה נשארת חדה.
...
export interface GenerateStructuredInput {
  ...
  images?: {
    ...
    audience?: string
    activity?: string
    /** Nano Banana merged "real product in scene" — primary for bigIdea. */
    productScene1?: string
    /** Different scene context — for deliverables. */
    productScene2?: string
    /** Different scene context — for closing or risks slides. */
    productScene3?: string
  }
...
function validateAndHealSlides(
  ...
  // 2) Image backfill — text-only slides on brands that have imagery available.
  // Priority: Nano Banana product scenes > scraped lifestyle/hero > scraped product (last resort, only as bg).
  const sa = input.scrapedAssets
  const scenes = [
    input.images?.productScene1,
    input.images?.productScene2,
    input.images?.productScene3,
  ].filter((u): u is string => !!u)
  const productPool = [...scenes, ...(sa?.heroImages || [])]
  const lifestylePool = [...(sa?.lifestyleImages || []), ...(sa?.heroImages || [])]
  // Raw scraped products are blurry catalog tiles — only use as faded bg, never as primary.
  const rawProductBgPool = sa?.productImages || []
  if (productPool.length || lifestylePool.length || rawProductBgPool.length) {
    let pIdx = 0, lIdx = 0, rawIdx = 0
    for (const s of pres.slides) {
      ...
      if (pool.length) {
        url = productSlide ? pool[pIdx++ % pool.length] : pool[lIdx++ % pool.length]
      } else if (s.layout === 'numbered-stats' && rawProductBgPool.length) {
        // Raw product as faded bg only (never as primary).
        url = rawProductBgPool[rawIdx++ % rawProductBgPool.length]
      }
      if (!url) continue
...
async function buildUserPrompt(input: GenerateStructuredInput): Promise<string> {
  ...
  if (hasScraped) {
    lines.push(`\n## <scraped_assets> — תמונות גולמיות מהאתר.`)
    lines.push(`**heroImages + lifestyleImages** הן full-bleed candidates. **productImages גולמיות הן catalog-style ולרוב נמוכות-איכות — אסור להשתמש בהן ישירות בשקף ראשי. אם productScene1/2/3 זמינות (Nano Banana) — חובה להעדיף אותן.**`)
    if (sa?.brandLogoUrl) lines.push(`- brandLogo: ${sa.brandLogoUrl}  (ירונדר אוטומטית, אל תכלול ב-slot)`)
    if (sa?.heroImages?.length) {
      lines.push(`- heroImages (${sa.heroImages.length}) — תמונות hero גדולות. שייכות ל-cover/closing/brief:`)
      sa.heroImages.forEach((u, i) => lines.push(`  - hero${i + 1}: ${u}`))
    }
    if (sa?.lifestyleImages?.length) {
      lines.push(`- lifestyleImages (${sa.lifestyleImages.length}) — תמונות אורח-חיים. שייכות ל-audience/brief/risks/sideImage של three-pillars-grid:`)
      sa.lifestyleImages.forEach((u, i) => lines.push(`  - lifestyle${i + 1}: ${u}`))
    }
    if (sa?.productImages?.length) {
      lines.push(`- productImages גולמיות (${sa.productImages.length}) — **last resort בלבד**. השתמש רק כ-backgroundImage על numbered-stats (יוצג ב-28% opacity), אסור כתמונה ראשית. עדיף שקף טקסטואלי על תמונת קטלוג נמוכת-איכות:`)
      sa.productImages.forEach((u, i) => lines.push(`  - productRaw${i + 1}: ${u}`))
    }
  ...
  if (input.images) {
    lines.push(`\n## <generated_images>`)
    // Nano Banana scenes get top billing — these are the deck-quality
    // composites of the real product into a styled scene.
    const hasScenes = input.images.productScene1 || input.images.productScene2 || input.images.productScene3
    if (hasScenes) {
      lines.push(`### productScenes (Nano Banana enriched — **PRIMARY for product slots**, deck-quality 1920×1080):`)
      if (input.images.productScene1) lines.push(`- productScene1 (foreground product, use in bigIdea): ${input.images.productScene1}`)
      if (input.images.productScene2) lines.push(`- productScene2 (hero shot, use in deliverables): ${input.images.productScene2}`)
      if (input.images.productScene3) lines.push(`- productScene3 (daily routine, use in closing/risks): ${input.images.productScene3}`)
    }
    lines.push(`### legacy AI images (fallback only — use ONLY when no productScene fits the slot):`)
