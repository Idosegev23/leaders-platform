...
- תקציב מופיע: אם הבריף כולל תקציב — הוא בתוצר עם CPE/CPM נגזרים. אין תקציב? "טרם הוגדר" — לא השמטה.
- מטרה↔מדד: לכל מטרה KPI ומנגנון מדידה (מכירות → קופון/UTM/דף נחיתה), לא רק reach/engagement.
- קהל חד: סגמנט התנהגותי צר, לא "כולם 25–65".
- מקור לכל סטטיסטיקה חיצונית: מקור בשם ושנה שתומך בטענה; אין אימות → הורד או "טרם אומת". אל תמציא ציטוט.
- אפס placeholder: אין @@/TBD/lorem/שם או handle חלקי. ערך אמיתי או השמטה.
</non_negotiables>`)
